...
DATASET_PATH = r"D:\Infosys Springboard\Cleaned Dataset\cleaned_email_dataset_all_types.csv"
...
def main():
    ...
    df = df.dropna(subset=["cleaned_full_text", "type"])

    df["label"] = df["type"].map(LABEL_MAP)

    # Stratified split
    train_texts, val_texts, train_labels, val_labels = train_test_split(
        df["cleaned_full_text"].tolist(),
        df["label"].tolist(),     # ✅ CORRECT (numeric labels)
        test_size=0.2,
        random_state=SEED,
        stratify=df["label"]
    )
